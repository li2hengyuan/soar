package com.heng.soar.modelCore.shiro;
...
/**
 * shiro的认证最终是交给了Realm进行执行 所以我们需要自己重新实现一个Realm，此Realm继承AuthorizingRealm Created
 * by sun on 2017-4-2.
 */
public class MyShiroRealm extends AuthorizingRealm {
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authenticationToken)
		...
		User user = uService.getUserByName(token.getUsername());
		if (user != null) {
	...
	protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
		...
		User user = uService.getUserByName(loginName);
		logger.info("权限认证!");
		if (user != null) {
			// 权限信息对象info，用来存放查出的用户的所有的角色及权限
			SimpleAuthorizationInfo info = new SimpleAuthorizationInfo();
			List<Role> roles = uService.getRolesByUser(user);
